...
public class CustomerController {
	...
	public ResponseEntity<Customer>  addCustomer(@Valid @RequestBody Customer customer,
		...
		try
		{
			...
			//to do customer validator
			//call region controller to get regionId based on city,state and country
			//customer.regionId to be filled
			customer.setRegionId(
					restTemplate.getForObject("http://localhost:9898/v1/cm/regions/"+
							customer.getCity()+"/"+customer.getState()+"/"+
							customer.getCountry(), Integer.class)
			);
	...
	@ApiOperation (value = "Find Customer By Mobile",
			produces = "Customer Mobile",
			response = Customer.class,
			tags = "findCustomerByMobile",
			notes = "http://localhost:9003/rvy/api/cm/v1/customers/9777172224"
	)
	// http://localhost:9003/cc/c1/getid/5012
	@GetMapping("/customer/{mobile}")
	public ResponseEntity<OrderResponse>  getCustomers(@PathVariable("mobile") Long mobile) throws CustomerEntityException{
		try {
			OrderResponse orp =  customerService.getCustomerByMobile(mobile);
			return new ResponseEntity<>(orp,HttpStatus.OK);
		}catch(ResponseStatusException e) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND,e.getMessage());
		}
	}
